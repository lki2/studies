	...
	public  class CurrentDate : CommonCommand
	{
		...
		public override byte[] getBytes(int counter)
		{
			...
			try
			{
                ...
                {
                    byte[] date = (new NullTerminatedString(DateTime.Now.ToString("yyyy-MM-dd"))).getNullTerminatedString();
                    ...
                    using (var ps = new StreamWriter(outputStream))
                    {
                        ...
                        ps.Write(System.Text.Encoding.UTF8.GetString(date).ToCharArray());
                    }
