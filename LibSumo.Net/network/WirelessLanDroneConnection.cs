...
namespace LibSumo.Net.lib.network
{
	...
	public class WirelessLanDroneConnection : DroneConnection
	{
        ...
        private static readonly log4net.ILog LOGGER = log4net.LogManager.GetLogger(typeof(WirelessLanDroneConnection));
		...
		private void runResponseHandler()
		{
			...
			new Thread(() =>
			{
				...
				try
				{
					...
					{
						LOGGER.Info(String.Format("Listing for response on port {0}", devicePort));
						int pingCounter = 0;
						while (true)
						{							                                                       
                    ...
                    using (var sumoSocket = new UdpClient())
					{
						while (true)
						{
							try
							{
                                ...
                                int counter = changeAndGetCounter(command);
                                byte[] packet = command.getBytes(counter);
